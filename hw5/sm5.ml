  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      (* append location to new reachable_locs recursively checking memory *)
      let rec append l m mnew =
        if List.mem l mnew then mnew
        else if List.mem_assoc l m then
          (
            let elem = List.assoc l m in
            match elem with
            | L l' -> append l' m (l::mnew)
            | R r -> iter_record r m (l::mnew)
            | _ -> l::mnew
          )
        else l::mnew
      (* append locations in record *)
      and iter_record r m mnew =
        match r with
        | [] -> mnew
        | (_, l)::r' -> iter_record r' m (append l m mnew)
      and iter_memory l m mnew =
        let rec check_memory (name, offset) mnew_ori mnew_left =
          match mnew_left with
          | (mname, moffset)::mnew' -> if name = mname then (append (name, offset) m mnew) else (check_memory (name, offset) mnew_ori mnew')
          | [] -> mnew_ori
        in
        check_memory l mnew mnew
      in
      let rec sweep_locs s e k m mnew =
        (
          match s with
          | (V (L l))::s' -> sweep_locs s' e k m (append l m mnew)
          | (V (R r))::s' -> sweep_locs s' e k m (iter_record r m mnew)
          | (M (_, Loc l))::s' -> sweep_locs s' e k m (append l m mnew)
          | h::s' -> sweep_locs s' e k m mnew
          | [] ->
            (
              match e with
              | (v, Loc l)::e' -> sweep_locs s e' k m (append l m mnew)
              | h::e' -> sweep_locs s e' k m mnew
              | [] ->
                (
                  match k with
                  | (c', e')::k' -> sweep_locs s e' k' m mnew
                  | [] -> mnew
                )
            )
        )
      ...
      let rec sweep_memory mori mleft mnew =
        match mleft with
        | (l, _)::m' -> sweep_memory mori m' (iter_memory l mori mnew)
        | [] -> mnew
      in

      (* let _ = reachable_locs := [] in *)
      let _ = reachable_locs := sweep_memory m m (sweep_locs s e k m []) in
