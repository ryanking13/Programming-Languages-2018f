...
struct
    ...
    | EQ ->
      (
        fun (v1,v2) ->
          ...
          (
            match v2 with
            | Int i2 -> Bool (getInt v1 = getInt v2)
            | _ -> raise (TypeError "type mismatch")
          )
          | Bool b1 ->
          (
            match v2 with
            | Bool b2 -> Bool (getBool v1 = getBool v2)
            | _ -> raise (TypeError "type mismatch")
          )
          | String s1 ->
          (
            match v2 with
            | String s2 -> Bool (getString v1 = getString v2)
            | _ -> raise (TypeError "type mismatch")
          )
          | Loc l1 ->
          (
            match v2 with
            | Loc l2 -> Bool (getLoc v1 = getLoc v2)
            | _ -> raise (TypeError "type mismatch")
          )
          | _ -> raise (TypeError "type mismatch")
      )
  ...
  let rec eval env mem exp = 
    ...
    | LET (d, e1) ->
      (
        match d with
        | VAL (x, e2) -> eval env mem (APP (FN (x, e1), e2))
        | REC (f, x, e2) ->
          (* FN case only returns Fun not RecFun T.T *)
          (* eval env mem (APP (FN (f, x, e1), e2)) *)
          eval (env @+ (f, Closure (RecFun (f, x, e2), env))) mem e1
      )
    | MALLOC e ->
      let (v, m') = eval env mem e in
      let (l, m'') = malloc m' in
